...
class SwPie extends Component{
	render() {
		...
		let Lobj2=[]
		if(sw_pieData){
			for(let i in sw_pieData.YAixs){
				var obj={}
				obj.name=i;
				obj.data=sw_pieData.YAixs[i]
				Lobj2.push(sw_pieData.YAixs[i]);
			}
    	...
    	let configPie = {
			...
			xAxis: {//x坐标
				categories: sw_pieData ? sw_pieData.XAixs:'',
				min:0,
				gridLineWidth: 0,
                tickInterval: 2,
                tickLength: 1,
	        ...
	        series:sw_pieData != undefined ? Lobj2 : initobj
        }
        return (
        	<div>
